...
public class MeshGenerator : MonoBehaviour
{
	...
	public void GenerateMesh (int[,] map, float squareSize)
	{
		...
		for (int i = 0; i < vertices.Count; i++) {
			float percentX = Mathf.InverseLerp (-map.GetLength (0) * squareSize / 2, map.GetLength (0) * squareSize / 2, vertices [i].x) * tileAmount;
			float percentY = Mathf.InverseLerp (-map.GetLength (0) * squareSize / 2, map.GetLength (0) * squareSize / 2, vertices [i].z) * tileAmount;
